...
Usage: python3 scripts/check_npp192.py [SHEET] [--sql]
  --sql prints the SQL calls that rebuild the sheet in a costing (used to
  write supabase/tests/15_acceptance_npp192.sql).
"""
import csv
import re
import sys
...
BOM_CATEGORY = {"BUSBAR": "busbar", "ENCLOSURE": "enclosure_parts"}


def print_sql(lines):
    """One call per workbook line: catalogue parts by code, the rest typed in."""
    print("-- generated by: python3 scripts/check_npp192.py OPTION1 --sql")
    for ln in lines:
        section = ln["section"].replace("'", "''")
        if ln["how"].startswith("workbook"):
            cat = BOM_CATEGORY.get(ln["section"], "switchgear")
            print(f"select app.add_manual_item(:'panel_id'::uuid, '{ln['ref'].replace(chr(39), chr(39)*2)}', "
                  f"'{cat}', {ln['sheet_unit']:.2f}, {ln['qty']:g});  -- {section}")
        else:
...
if __name__ == "__main__":
    ...
    sheet = args[0] if args else "OPTION1"
