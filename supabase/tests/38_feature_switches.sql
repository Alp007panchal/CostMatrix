...
select test.eq((select count(*)::int from public.features), 13,
  'thirteen advanced features are on the register');
...
commit;
select test.eq(:listed, 13, 'a costing engineer can see what exists');
