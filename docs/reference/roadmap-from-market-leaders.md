...
**Status:** planning reference, written 10 Sep 2026, **rows marked against what is actually built on 16 Sep 2026**. Companion to `current-costing-and-quotation-reference.md` (the "reference document"; its §5 decisions remain binding) and `ai-assistant-spec.md`.

**What is left, in one line.** Phase 2 is complete. Of phase 3 only **3.5** is unstarted, and it waits on the Opsmatrix timesheet feed, which is outside this repository. Of phase 4, **4.4** and **4.5** are unstarted. Everything else on this page is built — a row without ✅ is a row still to do, and that is the only thing this page is now for.
...
Each item is a small schema change plus, where noted, a minimal screen. None of them changes the pricing engine's behaviour today; they make the phase 2–4 features additive instead of disruptive.

**All of F1–F12 are built** (migrations 0100–0102 and 0106) and the section is kept as the record of why each field exists, not as a list of work.
...
**Reading the tables below:** ✅ built and on production · ◐ partly built, with what is left named in the row · no mark, not started.

Ordered by dependency and value. Each is one Claude Code session and one PR, as per the working agreement.

**Phase 2 is complete and on production**, every feature behind a switch that is off until the
master administrator turns it on (migration 0117). `advanced` is no longer a separate track: it is
the copy staging runs.

| # | Feature | Borrowed from | Depends on |
|---|---|---|---|
| 2.1 ✅ | **Foundations migration set (F1–F11)** (built, migrations 0100–0102), with seed back-fill and tests; no visible change except new optional fields on component and kit screens | — | — |
| 2.2 ✅ | **Price-list update workflow** (built, migration 0105): upload a supplier list (CSV/XLSX/PDF), map columns, match to components, show a side-by-side diff (old → new price, % change), accept per row or all; writes `component_price_history` | EPLAN Data Portal, CPQ price books | F1, F5, F9 |
| 2.3 ✅ | **BOM import** (built, migration 0107): upload a parts list from EPLAN, a consultant or a customer (CSV/XLSX), match part numbers and manufacturer numbers to the catalogue, propose kits where a matched device is a kit's main device, flag unmatched rows for manual choice or placeholder creation; result lands as a draft panel line item with `origin = import` | EPLAN BOM export | F1, F2, F4, F9 |
| 2.4 ✅ | **AI assistant phase 1** (built, migrations 0103 and 0104): enquiry → draft costing, pre-approval review, per `ai-assistant-spec.md` | — | F4, F5, F6, F11 |
| 2.5 ✅ | **Approval rules engine** (built, migration 0108) with the company's first real rules (minimum margin, placeholder parts, price age) and a "why this needs approval" panel on the costing | CPQ approval workflows | F7, F8 |
| 2.6 ✅ | **Quotation validity and follow-up** (built, migration 0108): `valid_until`, expiry job, follow-up reminders, "re-issue with current prices" action that creates a new revision from a refreshed price snapshot | CPQ | F8 |
| 2.7 ✅ | **Options and alternatives** (built, migration 0109) on one quotation (Option 1 / Option 2 price blocks, optional line items shown but excluded from the total) | CPQ, current workbook | F4 |
| 2.8 ✅ | **Estimate vs actual labour** (built, migrations 0110 and 0111): manual entry of actual hours per panel line item and process type; variance report by kit group; "suggested new standard" column (no automatic change to standards) | Accubid / ProEst | F3 |
| 2.9 ✅ | **Costing grid view** (built, no migration: the grid edits through the panel editor’s own functions): the whole costing as one grid — panels across the columns, kits and components down the rows — for costing a board of many near-identical panels the way the old workbook did. Added 12 Sep 2026 from the owner's confirmed spec, `costing-grid-view.md`, with the mockup at `mockups/costing-grid-view.html`. No engine or pricing change: every edit goes through the same functions as the panel editor | Current workbook, Accubid takeoff grids | F4 |

## 4. Phase 3 — configurator and learning (3–6 months)

| # | Feature | Borrowed from | Depends on |
|---|---|---|---|
| 3.1 ✅ | **Guided LV board configurator** (built 2026-09-12, migration 0115): a short question flow (supply sources, incomer ratings and type, changeover/sync, solar, feeder schedule by rating and quantity, APFC kVAr, metering, form, IP, access, cable entry) that fills `panel_line_items.parameters` and proposes kits with quantities; the engineer edits the result. Standard boards in minutes; non-standard boards still assembly-by-assembly | Schneider Power Build, ABB e-Configure, SIVACON | F2 (tags), F4 |
| 3.2 ✅ | **APFC configurator** (built 2026-09-12, migration 0113): target kVAr and step pattern → step kits and quantities. Built from whole quantities of the existing step kits rather than `qty_expression` as this row first guessed: a bank is a number of each size, and the kits are already in the library | Decision 4's "later phase" | F2 |
| 3.3 ✅ | **Parameterised kits** (built 2026-09-12, migration 0112) in the engine (`qty_expression` evaluated against `kit_parameters`); busbar metres as a kit parameter so a non-standard board adjusts metres in one place | EPLAN macros, Accubid assemblies | F2 |
| 3.4 ✅ | **Compatibility checks** (built 2026-09-12, migration 0114): rules on kits/components (frame vs enclosure depth, accessory fits device, feeder total vs incomer rating sanity check) surfaced as warnings, also used by the AI review | EPLAN parts checks | F1 attributes, F2 rules |
...
| 3.8 ✅ | **Panel layout pop-up** — specified 12 Sep 2026 and confirmed: `docs/reference/panel-layout-spec.md` (sections, the four mounting designs, double-front boards §2.1, the pop-up, outputs, rules, tests), `docs/reference/sivacon-s4-construction.md` (the S4 facts and how they map), and the mockups in `docs/reference/mockups/panel-layout-*.html` — the high-fidelity front, rear, 3D and double-front ones set the visual standard the finished feature is judged against. Every panel drawn at true scale so the enclosure can never be under-sized; the layout decides the sections and writes them into the enclosure line, and prints as the GA sketch in Annexure IV. **Stages one and two are built** (migrations 0119, 0120 and 0121): the fields, all five views — front, rear, plan, door and isometric — the arrangement rules, per-section settings for depth, busbar side, form and cable alley, double-front sections judged face by face, the verdict per section and the enclosure line. The **GA drawing is built too** — as **Annexure V**, since IV is the technical offer (D-284) — with the device tags shared with that offer. The **rear elevation** is built too (13 Sep 2026): a panel whose layout has a face at the back gets a second Annexure V sheet — the same board walked round, sections right to left, each busbar chamber on the other hand, face B's devices under the tags the front sheet already gave them. **3.8 is complete**; what remains is auto-arrange refinements once a person has driven it against real boards. | Rittal RiPanel, EPLAN Pro Panel | F12, F2 |
| 3.7 ◐ | **AI assistant phase 2** — **two of three built 16 Sep 2026, both pull requests open**: the assistant drafts a quotation's cover letter on the Release page as a `quotation_wording` proposal the approver carries into the form by hand (migration 0133, PR 79), and the master administrator switches the assistant on **for any company**, which is what lets an external company have it at all (migration 0134, PR 80). **Left: natural-language questions over the company's own data** — today a question is scoped to the one costing or enquiry on screen and the assistant says so. None of 3.7 can answer until there is credit on the Anthropic account | — | 2.4 |
...
| 4.3 ✅ | **Word and EPLAN exports** (built 2026-09-13, migration 0122): the EPLAN project name and drawing numbers on the costing with a paste-in importer that reads key/value, two-column or tabbed exports and proposes before it writes; the technical offer as an editable `.docx`; and a parts list of every frozen line for parts-management import, carrying the **device tags the panel layout draws** — so a part in EPLAN, a device on the GA drawing and a bullet in the technical offer are the same thing. A revision carries the drawing numbers; a copy starts with its own. Not verified against a real EPLAN installation — there is none here, so the column list is documented and mapped once on their side | EPLAN |
