...
| Phase 2.2 — Supplier price lists | **Built, PR open into `advanced`.** Migration 0105: four-attempt matching, a preview that writes only `import_jobs`/`import_rows`, accept per row or all, discard, and the price history recording its source. The Price lists screen reads CSV, Excel or a PDF (through `extract-document`), guesses the columns, and shows old → new with the percentage; rows that need a person are listed with the reason. Master admin for the master catalogue, company admin for private parts. 658 database assertions, 205 web tests, NPP-192 unchanged. | 2026-09-11 |
| Phase 2.3 — BOM import | **Next**: a parts list from EPLAN, a consultant or a customer matched to parts and kits, unmatched rows to a manual choice or a placeholder, landing as a draft panel with `origin = import`. | — |
| Phase 2.5 / 2.6 — Approval rules and validity | **After that**: `evaluate_approval_rules` wired into submit and approve with a rules editor and a "why this needs approval" panel; then expiry of `valid_until`, a follow-up when a sent quotation expires, and re-issue at current prices. | — |
