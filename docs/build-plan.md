...
| F1–F3 — Foundations PR A | **Built, PR open into `advanced`.** Migration 0100: component supplier, attributes, obsolescence, datasheet, lead time and price provenance; kit version, customer wording, labels, parameters and `qty_expression`; the kit version recorded on every costing line; a productivity factor per panel; actual hours; labour rate history. The optional fields are on the component and kit forms. 413 database assertions (38 new in test 23), 81 web tests. The kit-composition freeze turned out to be already built (D-177), so F2 needed no back-fill. | 2026-09-11 |
| F4–F11 — Foundations PR B | **Next.** Costing parameters and provenance, the generic `documents` table with text extraction, `activity_log`, `approval_rules`, validity, the import framework, `company_options` and the assistant tables. Migrations 0101 and 0102. | — |
