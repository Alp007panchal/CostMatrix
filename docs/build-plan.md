...
| F4–F11 — Foundations PR B | **Built, PR open into `advanced`.** Migration 0101: panel `parameters`, `origin` and `origin_ref` on every costing line, one `documents` table replacing `enquiry_attachments`, `activity_log`. Migration 0102: `approval_rules` with one default rule per company and a read-only engine, `valid_until` and `price_snapshot_at`, `import_jobs`/`import_rows` filled from the batches, `company_options` with the assistant switched off everywhere, the three assistant tables. The `extract-document` Edge Function reads PDF, Word, Excel and text. Files strip on the costing screen. 483 database assertions, test 15 unmodified. | 2026-09-11 |
| AI assistant PR A | **Next**, after foundations are merged: provider interface, Anthropic adapter, tools, context builder, agent loop, the server route as an Edge Function (D-180), logging, budget and rate limits; spec tests 3, 5, 6. | — |
